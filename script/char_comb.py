...
def runCombIn1Out1(targetLib, targetCell, expectationList2, unate):
  ...
  for trial in range(len(expectationList2)):
    ...
    tmp_Harness.set_spicef("vt_"+str(targetLib.vdd_voltage)+"_"+str(targetLib.temperature)+"_"\
    ...
    runSpiceCombDelayMultiThread(targetLib, targetCell, tmp_Harness)
...
#      +"_"+str(targetCell.outports[0])+str(tmp_outp0_val)
    tmp_Harness.set_spicef("vt_"+str(targetLib.vdd_voltage)+"_"+str(targetLib.temperature)+"_"\
      +"delay1_"+str(targetCell.cell)+"_"+str(targetCell.inports[1])\
      +str(tmp_inp0_val)+"_"+str(targetCell.inports[1])+str(tmp_inp1_val)\
      +"_"+str(targetCell.outports[0])+str(tmp_outp0_val))
    # run spice and store result
    #runSpiceCombDelayMultiThread(targetLib, targetCell, tmp_Harness, spicef)
...
def runSpiceCombDelayMultiThread(targetLib, targetCell, targetHarness):
...
#  results_i_vss_leak   = dict()
  ...
  for tmp_load in targetCell.load:
    for tmp_slope in targetCell.slope:
      ...
      targetHarness_tmp.set_slope_mag(targetLib)
      t1 = threading.Thread(target=lambda q, *arg1 : q.put(runSpiceCombDelaySingle(*arg1)),\
                           args=(que_comb, pool_sema, threadid, \
...
#      threadid += 1
  ...
  while not que_comb.empty():
    ...
    for tmp_load in targetCell.load:
      j=0
      for tmp_slope in targetCell.slope:
        #print(str(i)+" "+str(j)+" "+str(rslt['load'])+" "+str(tmp_load)+" "+str(rslt['slope'])+" "+str(tmp_slope))
        if((rslt['load'] == tmp_load) and (rslt['slope'] == tmp_slope)):
          #print(str(i)+" "+str(j)+" "+str(rslt['tran'])+" "+str(rslt['prop'])+" "+str(rslt['eintl'])+" "+str(rslt['ein']))
...
#    tmp_list_pleak =   []
#    for tmp_slope in targetCell.slope:
#      tmp_list_prop.append(results_prop_in_out[str(threadid)])
#      tmp_list_tran.append(results_trans_out[str(threadid)])
#
...
#      ## smaller Qs: intl. Q
#      ## Eintl = QsV
#      if(abs(results_q_vdd_dyn[str(threadid)]) < abs(results_q_vss_dyn[str(threadid)])):
#        res_q = results_q_vdd_dyn[str(threadid)]
#      else:
#        res_q = results_q_vss_dyn[str(threadid)]
#      tmp_list_eintl.append(abs(res_q*targetLib.vdd_voltage*targetLib.energy_meas_high_threshold \
#        - abs((results_energy_end[str(threadid)] - results_energy_start[str(threadid)])*((abs(results_i_vdd_leak[str(threadid)]) \
#        + abs(results_i_vss_leak[str(threadid)]))/2)*(targetLib.vdd_voltage*targetLib.energy_meas_high_threshold))))
#
#      ## input energy
#      tmp_list_ein.append(abs(results_q_in_dyn[str(threadid)])*targetLib.vdd_voltage)
#
#      ## Cin = Qin / V
#      tmp_list_cin.append(abs(results_q_in_dyn[str(threadid)])/(targetLib.vdd_voltage))
#
#      ## Pleak = average of Pleak_vdd and Pleak_vss
#      ## P = I * V
#      tmp_list_pleak.append(((abs(results_i_vdd_leak[str(threadid)])+abs(results_i_vss_leak[str(threadid)]))/2)*(targetLib.vdd_voltage)) #
#      threadid += 1
#
...
def runSpiceCombDelaySingle(pool_sema, threadid, targetLib, targetCell, targetHarness):
#                      spicef, \
#                      tmp_slope, tmp_load, tmp_slope_mag, \
#                      results_prop_in_out, results_trans_out,\
#                      results_energy_start, results_energy_end,\
...
#    slope_line = ".param slope ="+str(tmp_slope*tmp_slope_mag*targetLib.time_mag)+"\n"
...
#    spicefoe = str(spicef)+"_"+str(tmp_load)+"_"+str(tmp_slope)+"_energy.sp"
    ...
    # DEBUG
    #print("runSpiceCombDelaySingle")
    #print( targetHarness.get_spicefo_comb() )
    ## 1st trial, extract energy_start and energy_end
    res_prop_in_out, res_trans_out, res_energy_start, res_energy_end, \
      = genFileLogic_trial1(targetLib, targetCell, targetHarness, "delay") 
      #= genFileLogic_trial1(targetLib, targetCell, targetHarness, 0, cap_line, slope_line, temp_line, "none", "none", spicefo)
    ...
    res_prop_in_out, res_trans_out, \
      ...
      = genFileLogic_trial1(targetLib, targetCell, targetHarness, "energy") 
      #= genFileLogic_trial1(targetLib, targetCell, targetHarness, 1, cap_line, slope_line, temp_line, estart_line, eend_line, spicefoe)
...
#    results_i_vdd_leak[threading.current_thread().nam]  = res_i_vdd_leak
#    results_i_vss_leak[threading.current_thread().name]  = res_i_vss_leak
    ...
    rslt['prop']  = res_prop_in_out
    rslt['tran']  = res_trans_out
    ## intl. energy calculation
    ## intl. energy is the sum of short-circuit energy and drain-diffusion charge/discharge energy
    ## larger Ql: intl. Q, load Q 
    ## smaller Qs: intl. Q
    ...
    if(abs(res_q_vdd_dyn) < abs(res_q_vss_dyn)):
      ...
      res_q = res_q_vss_dyn
    rslt['eintl']  = abs(res_q*targetLib.vdd_voltage*targetLib.energy_meas_high_threshold \
    ...
    ## input energy
    rslt['ein'] = (abs(res_q_in_dyn) * targetLib.vdd_voltage)
    ## Cin = Qin / V
    rslt['cin'] = (abs(res_q_in_dyn) / targetLib.vdd_voltage)
    ## Pleak = average of Pleak_vdd and Pleak_vss
    ## P = I * V
    rslt['pleak']=(((abs(res_i_vdd_leak)+abs(res_i_vss_leak))/2) * (targetLib.vdd_voltage)) #
...
#def genFileLogic_trial1(targetLib, targetCell, targetHarness, sim_mode, cap_line, slope_line, temp_line, estart_line, eend_line, spicef):
def genFileLogic_trial1(targetLib, targetCell, targetHarness, sim_mode):
  #print (spicef)
  #print (estart_line)
  #print (eend_line)
  #print (spicef)
  ...
  spicef = targetHarness.get_spicefo_comb()
  if(sim_mode == "delay"):
    spicef = targetHarness.get_spicefo_comb()
  elif(sim_mode == "energy"):
    spicef = targetHarness.get_spicefo_comb_energy()
  else:
    print("meas_enery is not 0/1")
  ...
  if(os.path.isfile(spicelis)):
    ...
    with open(spicef,'w') as f:
      ...
      outlines.append(".param _tstart = slope\n")
      outlines.append(".param _tend = '_tstart + _tslope'\n")
      outlines += targetHarness.get_spice_lines_comb(targetLib, targetCell)
      ...
      ## energy measurement 
        ...
        outlines += targetHarness.get_spice_lines_comb_energy(targetLib, targetCell)
        outlines.append("* \n")
        ...
        targetLib.print_msg("Error, sim_mode should delay or energy")
        my_error()
