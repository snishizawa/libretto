  ...
  for tmp_slope in targetCell.slope:
    ...
    for tmp_load in targetCell.load:
      ...
      if(abs(res_q_vdd_dyn) < abs(res_q_vss_dyn)):
        tmp_list_eintl.append(abs(res_q_vdd_dyn*targetLib.vdd_voltage*targetLib.energy_meas_high_threshold-abs((res_energy_end - res_energy_start)*((abs(res_i_vdd_leak)+abs(res_i_vss_leak))/2)*(targetLib.vdd_voltage*targetLib.energy_meas_high_threshold))))
        #targetLib.print_msg(str(abs(res_q_vdd_dyn*targetLib.vdd_voltage)))
      else:
        tmp_list_eintl.append(abs(res_q_vss_dyn*targetLib.vdd_voltage*targetLib.energy_meas_high_threshold-abs((res_energy_end - res_energy_start)*((abs(res_i_vdd_leak)+abs(res_i_vss_leak))/2)*(targetLib.vdd_voltage*targetLib.energy_meas_high_threshold))))
...
#     targetLib.print_msg(str(abs(res_q_vdd_dyn*targetLib.vdd_voltage)))
      ...
      ## Pleak = average of Pleak_vdd and Pleak_vss
      ## P = I * V
      tmp_list_pleak.append(((abs(res_i_vdd_leak)+abs(res_i_vss_leak))/2)*(targetLib.vdd_voltage)) #
